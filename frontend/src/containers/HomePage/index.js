...
export class HomePage extends React.PureComponent { // eslint-disable-line react/prefer-stateless-function
  ...
  render () {
    ...
    var bookmarks = this.props.bookmarks.bookmarks.filter((v) => !this.state.hide[v.id])
    if (this.state.fuzzySearch !== '') {
      var fuzzySearch = this.state.fuzzySearch.toLowerCase()
      bookmarks = bookmarks.filter((v) => fuzzyMatch(v.url.toLowerCase(), fuzzySearch) ||
        fuzzyMatch(v.title.toLowerCase(), fuzzySearch))
      bookmarks.sort((a, b) => {
        const aRank = a.url.toLowerCase().includes(fuzzySearch) ||
          a.title.toLowerCase().includes(fuzzySearch)
        const bRank = b.url.toLowerCase().includes(fuzzySearch) ||
    ...
    if (this.state.viewMode === 0) {
      listing = bookmarks.map((v) =>
        <Panel
          ...
          : v.url
            ...
            <Row>
              <Col>
                <div className='link-card-button-open' onClick={() => window.open(v.url, '_blank')} >
            ...
            ? <span>
              <Glyphicon className='link-card-button-delete-ban-circle' glyph='ban-circle' onClick={(e) => this.shouldDelete(e, -1)} />
                &nbsp;
              <Glyphicon className='link-card-button-delete-trash' glyph='trash' onClick={(e) => this.delete(e, v.id)} />
            </span>
            : <span>
              <Glyphicon glyph='volume-off' onClick={(e) => this.hide(e, v.id)} />
              &nbsp;&nbsp;
              <Glyphicon glyph='remove' onClick={(e) => this.shouldDelete(e, v.id)} />
            </span>
          }
                </div>
              </Col>
      ...
      bookmarks.sort((a, b) => {
        var left = a.host.toLowerCase().trim()
          .replace('http://www', '')
          .replace('https://www', '')
          .replace('http://', '')
          .replace('https://', '')
          .replace('www.', '')
        var right = b.host.toLowerCase().trim()
          .replace('http://www', '')
          .replace('https://www', '')
          .replace('http://', '')
          .replace('https://', '')
      ...
      listing = bookmarks.map((v) =>
        <Panel key={v.id} className='link-list'>
          <Grid fluid>
            <Row>
              <Col>
                <div className='link-list-title' onClick={() => window.open(v.url, '_blank')}>
                  { v.title.trim() !== '' ? v.title.trim() : v.url }
                  <span className='link-list-title-host'>{v.host} - {moment(v.created_at).fromNow()}</span>
                </div>
                <div style={{float: 'right'}}>
                  {
                    ...
                    : <span> <Glyphicon glyph='remove' onClick={(e) => this.shouldDelete(e, v.id)} /> </span>
                  }
                </div>
              </Col>
            </Row>
    ...
    return (
      <Grid>
        <Row>
          <Col>
            <div className='row-count'>
              <ToggleButtonGroup name='viewMode' type='radio' defaultValue={0} value={this.state.viewMode} onChange={this.changeViewMode}>
        ...
        <Row>
          <Col>
            <div className='row-count'>
              {bookmarks.length + ' bookmark' + (bookmarks.length > 0 ? 's' : '')}
            </div>
          </Col>
        </Row>
        <Row>
...
export default connect(s2p, d2p)(HomePage)
